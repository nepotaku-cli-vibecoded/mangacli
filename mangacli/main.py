...
MEME_FACTS = [
    ...
    "this app has more edge cases than a geometry textbook",
    "the scraper is held together by hopes, dreams, and regex",
    "your backlog is not a to-do list, it's a graveyard of good intentions",
    "the only thing faster than this reader is how fast the sites change their HTML",
    "this was built in a weekend. it shows.",
    "the CDN has better uptime than my sleep schedule",
    "chapter numbering is a social construct",
    "all your base are belong to us (the manga site's base URL, specifically)",
    "the volume grouping is as accurate as a blindfolded dart throw",
    "the developer spent more time on these memes than on error handling",
    "unicode errors? never heard of her",
    "the page probing strategy is \"throw requests at the wall and see what sticks\"",
    "this app is the definition of 'it works on my machine'",
    "the Wordpress REST API is doing more heavy lifting than the entire Python backend",
    "reading order is a suggestion, not a rule",
    "the only thing consistent about this app is the inconsistency",
    "fallback to fallback to fallback: the design philosophy",
    "this code has been described as 'aggressively functional'",
    "the temp folder is the closest thing to a long-term storage solution we have",
    "the manhwa detector was removed because it kept flagging its own reflection",
]
